...
namespace Luminix {
	...
	Window::Window(const WindowSetting& window_specs) : m_WindowSpecs(window_specs), m_WindowHandle(nullptr), m_PrimaryMonitorHandle(nullptr), m_Running(true)
	{
		...
		LX_ASSERT(m_WindowHandle != nullptr, "GLFWwindow* is null, glfwCreateWindow might have failed!");
		...
		LX_ASSERT(m_PrimaryMonitorHandle != nullptr, "GLFWmonitor* (m_PrimaryMonitorHandle) is null, glfwGetPrimaryMonitor might have failed!");
		...
		glfwSetWindowPosCallback(m_WindowHandle, [](GLFWwindow* window, int xpos, int ypos) {
			WindowSetting& window_data = *(WindowSetting*)glfwGetWindowUserPointer(window);
		...
		glfwSetWindowCloseCallback(m_WindowHandle, [](GLFWwindow* window) {
			LX_INFO("Program Elapsed Time : {0}", Time::GetElapsedTime());
	    });

		glfwSetErrorCallback([](int error_code, const char* description) {
			LX_INFO("GLFW Error ({0}): {1}", error_code, description);
	    });
