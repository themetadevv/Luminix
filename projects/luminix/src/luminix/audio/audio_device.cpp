...
namespace Luminix {
	bool GetAvailableSoundDevices(std::vector<std::string>& devices_list_ref) {
		...
		LX_ASSERT(devices != nullptr, "alcGetString Failed!");
	...
	bool AudioDevice::SetDevice(const char* device_name) {
		...
		LX_ASSERT(m_alcDevice != nullptr, "alcOpenDevice Failed!");
	...
	bool AudioDevice::SetContext() {
		if (m_alcContext) {
			...
			LX_INFO("Destroyed old sound device context!");
		}

		LX_ASSERT(m_alcDevice != nullptr, "sound device* is null");
		if (!m_alcDevice) return false;

		m_alcContext = alcCreateContext(m_alcDevice, nullptr);
		LX_ASSERT(m_alcContext != nullptr, "alcCreateContext Failed!");
		if (!m_alcContext) return false;

		ALCboolean context = alcMakeContextCurrent(m_alcContext);
		LX_ASSERT(context != ALC_FALSE, "alcMakeContextCurrent Failed!");
		if (!context) {
			alcDestroyContext(m_alcContext);
			m_alcContext = nullptr;
			return false;
