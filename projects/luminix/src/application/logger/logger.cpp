...
namespace Luminix {	
	...
	bool Logger::CreateCoreLogger() {
		if (m_sCoreLogger != nullptr) {
			std::cout << "Core logger already initialized. Duplicate initialization request ignored.\n";
			return false;
		}

		if (m_LogSinks.empty()) {
			std::cout << "Luminix::Logger::CreateCoreLogger() failed, Did you call Logger::Init() ?\n";
			return false;
		}
	...
	bool Logger::AddClient(std::string_view client_name) {
		if (m_sClients.contains(std::string(client_name))) {
			std::cout << "Client logger '{}' already exists. Creation request ignored." << client_name << '\n';
			return false;
		}

		if (m_LogSinks.empty()) {
			std::cout << "Luminix::Logger::CreateCoreLogger() failed, Did you call Logger::Init() ?" << client_name << '\n';
			return false;
		}
	...
	bool Logger::DeleteClient(std::string_view client_name) {
		...
		if (!m_sClients.contains(name)) {
			std::cout << "Luminix::Logger::DeleteClient failed: client logger '{}' does not exist!" << client_name << '\n';
			return false;
		}
