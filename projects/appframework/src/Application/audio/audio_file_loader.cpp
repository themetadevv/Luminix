...
namespace af::AudioLoader {
	bool LoadWave(std::string_view audio_path, WAVE_AUDIO* wave_audio_header, std::vector<char>& pcm) {
		if (!std::filesystem::exists(audio_path)) {
			AF_ERROR("Failed to locate '.wav' file '{}'. The file does not exists at the specified path!", audio_path);
			return false;
		}
		...
		if (!audio_file.is_open()) {
			AF_ERROR("Failed to load '.wav' audio file '{}'. file may be corrupted, or lack read permissions!", audio_path);
			return false;
		}
	...
	bool LoadMP3(std::string_view audio_path, MP3_AUDIO* mp3_file_info, int& pcm_size) {
		if (!std::filesystem::exists(audio_path)) {
			AF_ERROR("Failed to locate '.mp3' file '{}'. The file does not exists at the specified path!", audio_path);
			return false;
		}
		...
		if (load_mp3 != 0) {
			AF_ERROR("Failed to load '.mp3' audio file '{}'. file may be corrupted, or lack read permissions!", audio_path);
			return false;
		}

		if (!info.buffer) {
			AF_ERROR("null buffer*: {}", audio_path);
			return false;
		}
